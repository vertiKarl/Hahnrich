...
export default class Hahnrich extends Logger {
  ...
  constructor() {
    ...
    const keepAlive = setInterval(() => {
      // Keep Hahnrich alive
      if (!this.aliveCheck()) {
        this.info("Exiting Hahnrich, have a nice day! :)");
  ...
  loadPlugin(plugin: Plugin) {
    ...
    plugin.events.on("LOAD_SETTINGS", () => {
      ...
      plugin.settings = this.settingsHandler
        .getSettings()
    ...
    plugin.events.on("SAVE_SETTINGS", async () => {
      ...
      for (const key of Object.keys(plugin.settings)) {
        ...
        this.settingsHandler.changePluginSetting(
          plugin.name,
          key,
          plugin.settings[key]
        );
    ...
    plugin.events.on("RESTART", () => {
      this.info("Stopping plugin:", plugin.name);
      plugin.stop();
      this.info("Waiting 5 seconds to restart:", plugin.name);
      setTimeout(() => {
  ...
  async restartPlugin(
    ...
    this.info("Waiting for", plugin.name, "to stop.");
    await plugin.stop();
  ...
  startPlugin(plugin: Plugin) {
    const promise: Promise<void> = new Promise((resolve, reject) => {
      plugin
        .execute()
        .then(() => {
          this.info("Plugin", plugin.name, "started!");
          return resolve();
        })
        .catch((err) => {
  ...
  async unloadPlugin(plugin: Plugin) {
    this.info("Unloading plugin:", plugin.name);
    await plugin.stop();
    if (this.plugins.has(plugin.name)) {
      this.plugins.delete(plugin.name);
      this.info("Plugin " + plugin.name + " unloaded!");
    }
  }
  ...
  async init() {
    ...
    const dirs = (await fs.readdir(pluginDir, { withFileTypes: true })).filter(
      async (dirent) =>
        dirent.isDirectory() ||
        (dirent.isSymbolicLink() &&
          (await (async (): Promise<boolean> => {
            return new Promise(async (resolve) => {
              return resolve(
                (await fs.stat(path.join(pluginDir, dirent.name))).isDirectory()
              );
            });
          })))
    );
    ...
    for (const dirent of dirs) {
      this.info("Trying to import:", dirent.name);
      try {
        const file = await import(
          path.join(pluginDir, dirent.name, "/index.js")
        );
